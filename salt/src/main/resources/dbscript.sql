...
(
    ...
    id              VARCHAR(30) NULL ,
    flgKey          VARCHAR(30) NULL ,
    regDate         DATE DEFAULT SYSDATE
);
ALTER TABLE T_ACCESS_LOG
    ADD CONSTRAINT T_ACCESS_LOG_PK PRIMARY KEY (seq);
COMMENT ON TABLE T_ACCESS_LOG IS '[접속로그]';
COMMENT ON COLUMN T_ACCESS_LOG.seq IS '일련번호';
COMMENT ON COLUMN T_ACCESS_LOG.id IS '아이디';
COMMENT ON COLUMN T_ACCESS_LOG.flgKey IS '구분키';
COMMENT ON COLUMN T_ACCESS_LOG.regDate IS '등록일시';


DROP SEQUENCE SEQ_T_ACCESS_LOG;
CREATE SEQUENCE SEQ_T_ACCESS_LOG START WITH 1 INCREMENT BY 1 MINVALUE 0 NOMAXVALUE;
